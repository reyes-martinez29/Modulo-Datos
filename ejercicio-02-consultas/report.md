...
> Cada tiempo es el promedio de 3 repeticiones con `gc.collect()` antes de cada run.
> Todos los resultados validados como numéricamente equivalentes entre los tres engines.

| Query | Descripción | pandas | DuckDB | polars | Ganador | Equiv. |
|-------|-------------|-------:|-------:|-------:|:-------:|:------:|
| Q1 | Conteo por `country_code` | 0.057s | 0.010s | 0.012s | **duckdb** | ✓ |
| Q2 | Promedio, mínimo y máximo de `amount` por `category` | 0.072s | 0.020s | 0.019s | **polars** | ✓ |
| Q3 | Top 10 usuarios por suma de `amount` | 0.046s | 0.062s | 0.021s | **polars** | ✓ |
| Q4 | Transacciones fallidas agrupadas por hora del día | 0.080s | 0.021s | 0.015s | **polars** | ✓ |
| Q5 | Filtro: `amount > 500`, país MX o CO, últimos 30 días | 0.161s | 0.135s | 0.126s | **polars** | ✓ |
| Q6 | Categoría con más transacciones por país | 0.163s | 0.034s | 0.030s | **polars** | ✓ |
| Q7 | Usuarios con más de 5 transacciones fallidas | 0.047s | 0.015s | 0.008s | **polars** | ✓ |
| Q8 | Monto promedio diario por `category` | 1.669s | 0.124s | 0.158s | **duckdb** | ✓ |


**Resumen de ganadores:** polars ganó en 6 de 8 queries, DuckDB en 2, pandas en ninguna. Todos los resultados fueron validados como numéricamente equivalentes entre los tres engines.
...
En Q1, los tres runs de pandas fueron: `[0.0864, 0.0425, 0.0428]`. El primero tardó 0.0864s y los siguientes dos tardaron 0.0425s y 0.0428s — el primer run fue 2.0x más lento.
...
Tiempo total DuckDB: **0.062s**. Polars fue más rápido en esta query (0.021s) porque aplica el mismo patrón TOP_N internamente pero sin el overhead de parsear SQL.
...
```
PARQUET_SCAN       → 73,704 filas materializadas de 1,000,000 en 0.11s
...
El resultado es concreto: de 1,000,000 filas disponibles, el lector materializó solo **73,704 — el 7.4% del archivo**. El otro 92.6% nunca se cargó en RAM.
...
La diferencia de RAM entre pandas (65 MB), DuckDB (3.3 MB) y polars (0.7 MB) confirma exactamente esto: pandas cargó el archivo completo antes de filtrar, DuckDB y polars solo cargaron las filas que pasaron los filtros.
...
La diferencia de RAM en Q6 es la más extrema del benchmark: pandas usó **75 MB** vs DuckDB **0.01 MB** — una diferencia de 7,488x. La razón es que pandas necesita dos GroupBy separados (uno para contar, otro para promediar), un `idxmax()` y un merge — cuatro operaciones que construyen DataFrames intermedios en el heap. DuckDB resuelve todo en la pasada única que muestra el plan.
...
Tiempos medidos: pandas **0.047s**, DuckDB **0.015s**, polars **0.008s**. Polars fue **5.9x** más rápido que pandas y **1.9x** más rápido que DuckDB.
...
La diferencia de RAM lo confirma: pandas **4.1 MB**, polars **0.01 MB**. Polars cargó solo lo que necesitaba.
...
Tiempos: pandas **1.669s**, DuckDB **0.124s**, polars **0.158s**. DuckDB fue **13.5x** más rápido que pandas — la diferencia más grande de todo el benchmark.

La causa concreta: cuando pandas ejecuta `dt.normalize()` sobre 1,000,000 timestamps para truncarlos al día, crea un objeto `datetime` de Python por cada fila. Un millón de objetos Python en el heap, cada uno con su overhead de allocación y de referencia. El proceso es secuencial porque Python no puede paralelizar la creación de objetos en el heap. Resultado: **1.669s** y **83 MB de RAM**.

DuckDB ejecuta `DATE_TRUNC('day', timestamp)` como una operación vectorizada en C++ sobre los buffers Arrow, sin crear ningún objeto Python. Resultado: **0.124s** y **0.62 MB** — una reducción de 134x en memoria.

Polars también mejora sobre pandas (0.158s) porque `dt.truncate("1d")` también es vectorizado en Rust. Pero DuckDB es más rápido porque su optimizador SQL fusiona el truncado y el GROUP BY en una sola pasada, algo que el API de polars no hace automáticamente.
...
Tiempos: pandas **0.161s**, DuckDB **0.135s**, polars **0.126s**. El ratio entre el más lento y el más rápido es solo **1.3x** — la diferencia más pequeña del benchmark.
...
Donde sí difieren es en RAM: pandas usó **65 MB** — el archivo completo cargado antes de filtrar. DuckDB usó **3.3 MB** y polars **0.7 MB**, porque ambos aplican los filtros antes de materializar filas en RAM. Cuando los tiempos son similares y la RAM importa — en producción con múltiples queries concurrentes — DuckDB y polars tienen ventaja estructural.
